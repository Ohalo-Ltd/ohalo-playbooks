...
- `package-and-deploy-dxr.sh` – helper script that bundles the local
  `ohalo-ansible` tree, uploads it plus an inventory file to S3, and
...
All other variables must already be populated before packaging. 
...
./package-and-deploy-dxr.sh \
  ...
  --inventory ohalo-ansible/inventory-example-dxr-single-server.json \
...
1. Creates a timestamped `dxr-ansible-<timestamp>.tar.gz` from
   `ohalo-ansible` and stores it in this directory.
